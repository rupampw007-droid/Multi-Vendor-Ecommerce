...
const CreateShop = ({
  ...
  const bioWordCount =
    bioValue.trim() === '' ? 0 : bioValue.trim().split(/\s+/).length;
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-xs ${bioWordCount > 100 ? 'text-red-500' : 'text-gray-400'}`}
            >
              {bioWordCount}/100
            </span>
          </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className={`w-full rounded-md border px-3 py-2 text-sm outline-none transition-colors focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
              errors.opening_hours ? 'border-red-400' : 'border-gray-300'
            }`}
